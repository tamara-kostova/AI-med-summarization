...
class MkRobertaSummarizer(ExtractiveSummarizer):
    ...
    def _cluster_sentences(self, embeddings, sentences, num_sentences_to_return):
        ...
        if self.clustering_algorithm == "kmeans":
            clustering_model = KMeans(
                n_clusters=num_sentences_to_return, random_state=self.random_state
            )
            ...
            if self.use_medical_weighting:
                med_boost = np.array(
                    [1.0 + 0.5 * self._contains_medical_terms(s) for s in sentences]
                )
    ...
    def generate_extractive_summary_chunk(self, text):
        ...
        try:
            ...
            selected_indices = self._cluster_sentences(
                embeddings, sentences, num_sentences
            )

            if self.use_first and 0 not in selected_indices:
                selected_indices = (
                    [0] + selected_indices[:-1] if len(selected_indices) > 0 else [0]
                )
    ...
    def generate_extractive_summary(self, text, num_sentences_per_chunk=None):
        ...
        logger.info(
            f"Generating medical extractive summary with {num_sentences_per_chunk} sentences per chunk"
        )
        ...
        try:
            ...
            if len(summaries) > 1:
                logger.info(
                    f"Applying second-level medical summarization on {len(summaries)} chunks"
                )
