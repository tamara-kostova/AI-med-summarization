...
class SummaRuNNer(nn.Module):
    ...
    def forward(self, sent_embeddings, sentence_lengths, doc_length):
        ...
        sent_output, _ = nn.utils.rnn.pad_packed_sequence(packed_output, batch_first=True)
        
        doc_mask = torch.arange(max_doc_len).expand(batch_size, max_doc_len).to(sent_output.device)
        doc_mask = doc_mask < doc_length.unsqueeze(1)
        masked_output = sent_output * doc_mask.unsqueeze(2).float()
        
        doc_embedding = self.doc_rep(torch.sum(masked_output, dim=1) / doc_length.unsqueeze(1).float())
        
        content_scores = self.content(sent_output)
        
        salience_scores = self.salience(sent_output, doc_embedding.unsqueeze(1).expand(-1, max_doc_len, -1))
        
        position_idx = torch.arange(max_doc_len).expand(batch_size, max_doc_len).to(sent_output.device)
        ...
        for i in range(max_doc_len):
            if i > 0:
                sum_embeddings = sum_embeddings + sent_output[:, i-1, :] * probs[:, i-1].unsqueeze(1)
                novelty_scores[:, i, :] = self.novelty(sent_output[:, i, :].unsqueeze(1), sum_embeddings.unsqueeze(1)).squeeze(1)
            
            scores = content_scores[:, i, :] + salience_scores[:, i, :] + novelty_scores[:, i, :] + position_scores[:, i, :]
    ...
    def _get_sentence_embeddings(self, sentences: List[str]) -> torch.Tensor:
        ...
        with torch.no_grad():
            for sentence in sentences:
                ...
                sentence_embedding = outputs.last_hidden_state.mean(dim=1).squeeze().cpu()
                embeddings.append(sentence_embedding)
                
        return torch.stack(embeddings)
        
    def generate_extractive_summary(self, text: str, ratio: float = 0.2, 
                                  min_length: int = 40, max_length: int = 600) -> str:
        """
        ...
        for i, sent_embedding in enumerate(sentence_embeddings):
            ...
            salience_score = F.cosine_similarity(sent_embedding.unsqueeze(0), doc_embedding.unsqueeze(0)).item()
            
            position_score = np.exp(-i / len(sentences))
            
            if i > 0 and torch.norm(summary_embedding) > 0:
                novelty_score = 1 - F.cosine_similarity(sent_embedding.unsqueeze(0), summary_embedding.unsqueeze(0)).item()
            else:
                novelty_score = 1.0
        ...
        if len(summary) < min_length and len(sentences) > len(summary_sentences):
            ...
            remaining_indices_by_score = sorted(remaining_indices, key=lambda i: scores[i], reverse=True)
